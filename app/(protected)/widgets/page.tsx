    ...
    <div className="space-y-6">
      ...
      {widgets.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-md border border-dashed p-8 text-center">
          ...
          <Button variant="outline" onClick={() => setOpen(true)}>Создать виджет</Button>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {widgets.map((widget) => (
            <Card key={widget.id}>
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground">Уникальный ID: <span className="font-mono text-black">{widget.id}</span></p>
              </CardContent>
            </Card>
          ))}
        </div>
